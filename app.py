...
def fetch_options_data(ticker_symbol, expiry_offset=0):
    ...
    # Get current price
    try:
        spot = ticker.history(period="1d", interval="1m")["Close"].iloc[-1]
    except:
    ...
    if iv_coverage < 50:
        ...
        if fallback_iv is None:
            fallback_iv = estimate_iv_from_historical(ticker, days=30)
        if fallback_iv is None:
            fallback_iv = 0.15
...
hist_data = yf.Ticker(ticker_symbol).history(period="5d", interval="5m")
