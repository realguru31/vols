...
if len(gex_df_filtered) == 0:
    st.warning("No valid GEX data in selected range")
    st.stop()
...
with tab1:
    ...
    if net_gex > 0:
        st.success("Positive Net GEX: Dealers are net short gamma. Market likely to see dampened volatility.")
    else:
        st.warning("Negative Net GEX: Dealers are net long gamma. Market likely to see amplified moves.")
